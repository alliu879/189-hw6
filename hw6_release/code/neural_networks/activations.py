...
class SoftMax(Activation):
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        result = []
        for pt in probs:
            jacobian = np.outer(pt, pt) * \
            np.full((probs.shape[1], probs.shape[1]), -1) + np.diag(pt)
            result.append(jacobian)
        return np.squeeze(dY @ np.array(result))
